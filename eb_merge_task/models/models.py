    ...
    def apply_(self):
        ...
        if current.project_id:
            ...
            if len(current.work_ids) > 0:
                for s3 in current.work_ids:
                    if current.type == '1':
                        self.env['project.task.work'].browse(s3.id).write({'active': False, 'w_id': current.id})
                    else:
                        if current.choix == '2':
                            self.env['project.task.work'].browse(s3.id).write({'active': False, 'w_id': current.id})
                        else:
                            self.env['project.task.work'].browse(s3.id).write({'w_id': current.id})
                    ...
                    for kk in res_cpt:
                        if current.type == '1':
                            ...
                            if current.choix == '2':
                                ...
                                self.pool.get('project.task.work').browse(kk).write({'w_id': current.id})

        self.write({'state': 'open'})
        view = self.env['sh_message.sh_message_wizard']
        # if cr.dbname == 'DEMO222':
        #     found = self.pool.get('project.task.work').search(cr, uid, [('w_id', '=', current.id)])
        #
        #     if found:
        #         for jj in found:
        #             s2 = self.pool.get('project.task.work').browse(cr, uid, jj, context=context)
        #             ##raise osv.except_osv(_('Transfert impossible!'),_("Pas de Stock suffisant pour l'article %s  !")%s2.id)
        #             sq40 = ((
        ...
        #                         s2.id, int(time.mktime(
        #                             datetime.strptime(s2.date_start or s2.create_date[:10],
        #                                               '%Y-%m-%d').timetuple())) or '',
        #                         int(time.mktime(
        #                             datetime.strptime(s2.date_start or s2.create_date[:10],
        #                                               '%Y-%m-%d').timetuple())) or '',
        #                         1, s2.project_id.id, s2.name.encode('ascii', 'ignore').replace("'", "\\'"),
        #                         s2.project_id.id, s2.sequence or '', str(s2.project_id.partner_id.id) or '',
        #                         s2.product_id.name.encode('ascii', 'ignore').replace("'", "\\'") or '',
        #                         s2.categ_id.name.encode('ascii', 'ignore').replace("'", "\\'") or '',
        #                         str(s2.task_id.coordin_id.id) or '', str(s2.gest_id.id) or '', s2.poteau_t or 0,
        #                         int(time.mktime(
        #                             datetime.strptime(s2.date_start_r or '2000-01-01', '%Y-%m-%d').timetuple())) or '',
        #                         int(time.mktime(
        #                             datetime.strptime(s2.date_end_r or '2000-01-01', '%Y-%m-%d').timetuple())) or '',
        #                         str(s2.task_id.id) or '', '72', int(time.mktime(
        #                             datetime.strptime(s2.date_start or s2.create_date[:10],
        #                                               '%Y-%m-%d').timetuple())) or '',
        #                         int(time.mktime(
        #                             datetime.strptime(s2.date_end or '2000-01-01', '%Y-%m-%d').timetuple())) or '',
        #                         str(s2.employee_id.id if s2.employee_id else '') or '', str(s2.uom_id.id) or '', '',
        #                         str(s2.zone) or '', str(s2.secteur) or ''))
        #             sq41 = ((
